<template>
    <div id="conter">
        <div id="view" v-loading="loading">
            ...
            <el-row :gutter="16">
                <el-col v-on:click.native="golesson(item)" class="elCol" :span="6" v-for="(item, index) in suclesson"
                    :key="index">
...
export default {
    ...
    mounted() {
        setTimeout(() => {
            axios({
                method: 'get',
                url: 'http://localhost:8081/stuCourses/getOwnCourse?userId=' + JSON.parse(localStorage.getItem('users')).id,
                headers: {
