  ...
  <div class="box">
    <div class="register" v-loading="loading">
      <el-form ref="form" :model="form" :rules="rules" label-width="30px" class="from">
        ...
        <el-form-item label="" prop="psw">
          <el-input autocomplete="off" prefix-icon="el-icon-lock" placeholder="至少6位字母和数字" show-password
            v-model.number="form.psw"></el-input>
        </el-form-item>
        <el-form-item label="" prop="pswok">
          <el-input autocomplete="off" prefix-icon="el-icon-lock" placeholder="至少6位字母和数字" show-password
            v-model.number="form.pswok"></el-input>
        </el-form-item>
      ...
      <div class="login">已有账号，
        <router-link to="/login" style="margin-right: 30px;">登录</router-link>
        <el-link href="http://localhost:8081/#/yemian" type="primary" style="font-size: 14px;">返回主页</el-link>
      </div>
    </div>
  </div>
</template>
        ...
        this.$refs[form].validate((valid) => {
          if (valid) {
            ...
            axios({
              ...
              if (err.response.data.exception == 'com.ikun.eduproject.error.EmailException') {
                this.$notify({
                  title: '消息',
                  message: ('邮箱无法使用有误，请输入正确的邮箱'),
