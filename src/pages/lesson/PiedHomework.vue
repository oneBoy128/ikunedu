    ...
    <div id="conter">
      <div id="view" v-loading="loading">
        ...
        <el-table height="500" :default-sort="{ prop: 'updateTime' }" v-show="!isEmpty" :data="suclesson" stripe
          ...
          <el-table-column prop="credit" label="得分" width="200">
          </el-table-column>
          <el-table-column prop="assignmentUrl" label="作业文件" width="200">
            <template slot-scope="scope">
  ...
  export default {
    ...
    mounted() {
      setTimeout(() => {
        axios({
          method: 'get',
          url: 'http://localhost:8081/assignment/getByCourseIdOK?courseId=' + JSON.parse(localStorage.getItem('choselesson')).courseId,
          headers: {
            'Content-Type': 'application/json;charset=UTF-8'
          }
        }).then(resp => {
