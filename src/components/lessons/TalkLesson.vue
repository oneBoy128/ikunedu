<template>
    <div id="bottombox"><!--为了方便管理，把WebLesosn页的下部分抽出了这个模块-->
        <div id="left" v-loading="loading"><!--评论区-->
            <div class="infomation_tabs">
                <div class="infomation_tabs_tab" id="detail-tag-button">评论</div>
            </div>
            <div class="talk_tabs"><!--素质交流区,智学云钢琴家-->
                <div id="talk_tx">
                    <img :src="person.imageUrl" alt="用户头像" class="round-image">
                </div>
                <div style="width: 80%; margin-left: 20px;">
                    <div class="start" style="display: flex; position: relative; bottom: 10px;">评分：<el-rate
                            v-model="value"></el-rate></div>
                    <el-input type="textarea" :rows="4" :resize="'none'" placeholder="请输入你的评论" v-model="description"
                        class="textpl" maxlength="200" show-word-limit style="font-size: 18px;">
                    </el-input>
                    <el-button @click="subtalk" style="height: 85%;" type="primary">发表</el-button>
                </div>
            </div>
            <div style="margin-top: 42px;">
                <div v-if="this.mytalk !== undefined" style="margin-top: 25px;"><!--自己评论的容器-->
                    <p style="font-size: 20px;">我的评论</p>
                    <el-empty v-show="isEmpty" description="暂无评论，快来抢沙发"></el-empty>
            ...
            <p style="margin-top: 50px;font-size: 20px;" v-show="!isEmpty">全部评论（{{ number }}）</p>
            <div><!--存放评论的容器-->
                <el-empty v-show="isEmpty" description="暂无评论，快来抢沙发"></el-empty>
        ...
        <el-dialog title="评论信息" :visible.sync="dialogFormVisible">
            <div v-if="mytalk != null" style="display: flex;">评分：<span><el-rate v-model="mytalks.stars"></el-rate></span>
            </div>
...
export default {
    ...
    data() {
        return {
            ...
            username: JSON.parse(localStorage.getItem("users")).username,//获取自己的名字
            description: '',//个人评论
            talks: null,//所有评论
