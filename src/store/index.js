...
const state = {
    ...
    chosenlesson:{},//被选择的课程
    waitroles:[],//所有待审核的老师
    lessonCatgory:[],//所有课程类别
    
    code:0,//返回状态信息
    yanzheng: false,//购买课程前的验证
    serchcode:false,//查找状态
    ImgCode:null,//返回传输图像的code信息
    IMGURL:null,//返回图片的URL
    FILEURL:null,//返回文件的URL
};
const actions = {
    ...
    AllWait(context){
      axios({
        method:'get',
        url:'http://localhost:8081/user/getTeacherNo',
        headers: {
            'Content-Type': 'application/json;charset=UTF-8'
        }
      }).then(resp=>{
        context.state.waitroles = resp.data.data;
      }).catch(err=>{
        console.log('失败：',err)
      })
    },
    ...
    AllMajor(context){
      axios({
        ...
        context.state.lessonCatgory = resp.data.data;
        context.state.serchcode = true;
      }).catch(err=>{
        console.log('失败：',err)
      })
    },
    ...
    AdminLessons(context,value){
      axios({
        method:'post',
        url:'http://localhost:8081/course/updateChecked?courseId='+value.courseId+'&checked='+value.checked+'&reason='+value.reason,
        headers: {
          'Content-Type': 'application/json;charset=UTF-8'
        }
      }).then(resp=>{
